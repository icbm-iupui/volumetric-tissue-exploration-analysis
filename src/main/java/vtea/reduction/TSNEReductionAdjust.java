    ...
    public boolean process(ArrayList al, double[][] feature, boolean val) {
        ...
        if (val) {
            try {
                ...
                String s = getPython();
                String randScript = getCWD() + String.format("%1$csrc%1$cmain%1$cresources%1$cTSNERandom.py", File.separatorChar);
                ...
                deleteFiles(new String[]{"random_initial_values_for_tsne.csv", "matrix_for_python.csv"});
...
//        double[][] Y = run(config, (int)System.nanoTime(), eta);
    ...
    private double[][] run(TSneConfiguration parameterObject, double[][] randInit, double eta) {
         ...
         * calloc(N * K, sizeof(int)); _val_P = (double*) calloc(N * K, sizeof(double));
         */
        ...
        if (exact) {
            IJ.log("Done in " + IJ.d2s((end - start) / 1000.0, 2) + " seconds!\nLearning embedding...\n");
        } else {
            IJ.log("Done in " + IJ.d2s((end - start) / 1000.0, 2) + " seconds (sparsity = " + IJ.d2s((double) row_P[N] / ((double) N * (double) N), 2) + ")!\nLearning embedding...\n");
        }
