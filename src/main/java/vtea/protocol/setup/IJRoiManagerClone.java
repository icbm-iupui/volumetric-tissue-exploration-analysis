...
/** This weas lifted from the IMageJ soruce code: https://github.com/imagej/imagej1/blob/master/ij/plugin/frame/RoiManager.java*/

public class IJRoiManagerClone extends JPanel implements ActionListener, ItemListener, MouseListener, MouseWheelListener, ListSelectionListener, Iterable<Roi> {
...
//		instance = this;
	...
	public IJRoiManagerClone(boolean b) {
		//super("ROI Manager Clone");
		list = new JList();
		listModel = new DefaultListModel();
		list.setModel(listModel);
		errorMessage = null;
	...
	public void makeWindow() {
		...
		buttonpanel = new JPanel();
                buttonpanel.setBackground(vtea._vtea.BACKGROUND);
		int nButtons = BUTTONS;
		buttonpanel.setLayout(new GridLayout(nButtons, 1, 5, 0));
                //panel.add(scrollPane);
		addButton("Add");
		addButton("Update");
		addButton("Delete");
                addButton("Rename...");
                addButton("OR (Combine)");
		addButton("Open...");
		addButton("Save...");
		...
		buttonpanel.add(labelsCheckbox);
		...
		addPopupMenu();
            ...
            panel.add(scrollPane, layoutConstraints);
            ...
            layoutConstraints.weightx = 1;
            layoutConstraints.weighty = 1;
            panel.add(buttonpanel, layoutConstraints);
            
            //scrollPane.setPreferredSize(buttonpanel.getSize());
...
//		addPopupItem("Multi Crop");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (command.equals("Add"))
			...
			Point bloc = moreButton.getMousePosition();
...
//		if (record()) Recorder.record("roiManager", "Add & Draw");
	...
	boolean delete(boolean replacing) {
		...
		if (index.length==0 || (replacing&&count>1)) {
			...
			canceled = false;
			if (!IJ.isMacro() && !macro) {
				YesNoCancelDialog d = new YesNoCancelDialog(IJ.getInstance(), "ROI Manager", msg);
				if (d.cancelPressed())
...
//		if (record()) Recorder.record("roiManager", "Split");
	...
	boolean error(String msg) {
		new MessageDialog(IJ.getInstance(), "ROI Manager", msg);
		Macro.abort();
		return false;
	}
...
//			return "null";
			...
			//return	(String) listModel.getElementAt(i);
