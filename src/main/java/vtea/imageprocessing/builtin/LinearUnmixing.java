...
//            dComponents.add(0, (new JTextField(sRadius.getText(), 3))); 
    ...
    private Object[] unmix(float[][] spectra, float[][] data, int startch, int endch, boolean outresid, boolean outc2, boolean truncneg, boolean outerrs) {
        ...
        float[][] residstack = null;
        float[] c2stack = null;
        float[][] errstack = null;
        if (outresid) {
            residstack = new float[slices][channels];
        }
        if (outc2) {
            c2stack = new float[slices];
        }
        if (outerrs) {
            errstack = new float[slices][spectra.length];
        }
        for (int j = 0; j < slices; j++) {
            ...
            for (int l = 0; l < contributions.length; l++) {
                if (truncneg && contributions[l] < 0.0f) {
                    contributions[l] = 0.0f;
                }
        ...
        int outcounter = 1;
        if (outresid) {
            output[outcounter] = residstack;
            outcounter++;
        }
        if (outc2) {
            output[outcounter] = c2stack;
            outcounter++;
        }
        if (outerrs) {
            output[outcounter] = errstack;
            outcounter++;
        }
        return output;
    }
    ...
    /**
     * *****************************************************************************
     * Copyright (c) 2013 Jay Unruh, Stowers Institute for Medical Research. All
     * rights reserved. This program and the accompanying materials are made
     * available under the terms of the GNU Public License v2.0 which
 ...
 *****************************************************************************
	 ...
	 * Institute for Medical Research 4/25/08
        ...
        /**
         * *******
         * gets the slope for a linear fit with the first point being 0 and
         * increasing by 1
         *
         * @param data
         * @return
        ...
        /**
         * ***************************************
         * this function gets the amplitude and offset for any function
         * (fit[i]=amp*function[i]+offset)
         *
         * @param function
        ...
        /**
         * **************
         * this is like loess smooth except with a user defined function--find
         * the amp and offset at windowsize around each point
         *
         * @param function: the function
         * @param data
         * @param offset
         * @param windowsize
         * @return an array of coefficients (first one or two places) and the
         * fit
         */
        public double[][] get_running_amp_offset(double[] function, float[] data, boolean offset, int windowsize) {
            ...
            for (int i = halfwsize; i < (data.length - halfwsize); i++) {
                if (i > halfwsize) {
                    ...
                    sumx2 -= function[prev] * function[prev];
                    sumx2 += function[next] * function[next];
                    sumx -= function[prev];
                    sumx += function[next];
                    sumy -= data[prev];
                    sumy += data[next];
                    sumxy -= data[prev] * function[prev];
                    sumxy += data[next] * function[next];
                }
                if (offset) {
            ...
            for (int i = 0; i < halfwsize; i++) {
                profile[i] = profile[halfwsize].clone();
            }
            for (int i = (data.length - halfwsize); i < data.length; i++) {
                profile[i] = profile[data.length - halfwsize - 1].clone();
            }
            return profile;
        }
